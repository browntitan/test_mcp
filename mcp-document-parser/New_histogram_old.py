...
- Session = consecutive USER submissions where each new submission arrives within <= 10 minutes of the previous one.
  Session duration is measured from the first USER submission timestamp to the last ASSISTANT response timestamp
  associated with submissions in that session.
...
# ----------------------------
# JSON parsing: extract USER submission events as (user_ts, assistant_ts)
# ----------------------------

def extract_user_message_events(chat_obj: Any) -> Tuple[List[Tuple[datetime, Optional[datetime]]], Dict[str, int]]:
    """Extract USER submission events from the chat JSON.

    Returns a list of (user_ts, assistant_ts) pairs.

    - user_ts:
        * direct from the user message timestamp if present
        * otherwise inferred from the earliest child timestamp
    - assistant_ts:
        * inferred as the latest child timestamp (often the assistant response)

    This supports measuring session duration as: first user_ts -> last assistant_ts within the session.
    ...
    # Normalize messages by id for child lookups.
    ...
    events: List[Tuple[datetime, Optional[datetime]]] = []
    ...
    for mid, msg in normalized.items():
        ...
        # Direct user timestamp if available
        user_dt_direct = to_dt(msg.get("timestamp") or msg.get("createdAt") or msg.get("created_at") or msg.get("time"))

        # Children timestamps (often assistant responses)
        ...
        # Infer user timestamp from earliest child timestamp if needed
        if user_dt_direct is not None:
            user_dt = user_dt_direct
            stats["user_msgs_direct_ts"] += 1
        else:
            if child_dts:
                user_dt = min(child_dts)
                stats["user_msgs_inferred_ts"] += 1
            else:
                stats["user_msgs_skipped_no_ts"] += 1
                continue

        assistant_dt = max(child_dts) if child_dts else None

        events.append((user_dt, assistant_dt))
...
class RangeMetrics:
    ...
    # Total population (all users)
    mean_session_minutes_total: Optional[float]
    median_session_minutes_total: Optional[float]
    mean_sessions_per_week_total: Optional[float]
    median_sessions_per_week_total: Optional[float]
...
def analyze_time_range(
    ...
    zero_duration_sessions_adjusted = 0

    def close_session():
        nonlocal session_start, session_end, last_msg_ts, current_user, all_sessions, user_week_session_counts, zero_duration_sessions_adjusted
        ...
        if dur_min <= 0.0:
            # If a thread/session has only one submission (or timestamps collapse to the same second),
            # treat it as a minimum 4-minute interaction.
            dur_min = 4.0
            zero_duration_sessions_adjusted += 1
    ...
    for user_id, created_at, updated_at, chat_json in stream_chat_rows(conn, start_epoch, end_epoch, itersize=itersize):
        ...
        # Unique users should only count users with (a) a thread created Mon–Thu within the window OR
        # (b) at least one qualifying Mon–Thu user submission event within the window.
        # Do NOT count a user just because we saw an overlapping chat row.
        try:
            created_dt = from_epoch(float(created_at), epoch_unit)
        except Exception:
            created_dt = None
        if created_dt is not None and (start_dt <= created_dt < end_dt) and (created_dt.weekday() in INCLUDED_WEEKDAYS):
        ...
        # Extract user submission events as (user_ts, assistant_ts)
        events, st = extract_user_message_events(chat_json)
        ...
        # Filter to window + Mon-Thu based on user submission time, while retaining assistant response time
        filtered_events: List[Tuple[datetime, Optional[datetime]]] = []
        for user_ts, assistant_ts in events:
            if user_ts < start_dt or user_ts >= end_dt:
                continue
            if user_ts.weekday() not in INCLUDED_WEEKDAYS:
                continue

            # Normalize assistant_ts if present
            if assistant_ts is not None:
                if assistant_ts < user_ts:
                    assistant_ts = user_ts
                # Clamp assistant timestamp to the analysis window end to avoid spanning far beyond the period
                if assistant_ts > end_dt:
                    assistant_ts = end_dt

            filtered_events.append((user_ts, assistant_ts))

        if not filtered_events:
            continue

        # Condition (b): at least one qualifying Mon–Thu user submission in-window
        unique_users.add(user_id)

        filtered_events.sort(key=lambda x: x[0])
        extraction_stats["user_events_used_after_filters"] += len(filtered_events)

        # Record days-used per user-week (based on USER submission days)
        for user_ts, _assistant_ts in filtered_events:
            wk = monday_of(user_ts)
            key = (user_id, wk)
            if key not in user_week_days_used:
                user_week_days_used[key] = set()
            user_week_days_used[key].add(user_ts.weekday())

        # Sessionize USER submissions for this user.
        # Session boundaries are based on gaps between USER submissions.
        # Session duration uses last ASSISTANT response time when available.
        for user_ts, assistant_ts in filtered_events:
            end_candidate = assistant_ts if assistant_ts is not None else user_ts

            if session_start is None:
                session_start = user_ts
                session_end = end_candidate
                last_msg_ts = user_ts
                continue

            # Safety: if timestamps arrive out-of-order across threads, do not merge.
            if last_msg_ts is not None and user_ts < last_msg_ts:
                close_session()
                session_start = user_ts
                session_end = end_candidate
                last_msg_ts = user_ts
            ...
            if last_msg_ts is not None and (user_ts - last_msg_ts) <= gap:
                if session_end is None or end_candidate > session_end:
                    session_end = end_candidate
                last_msg_ts = user_ts
            else:
    ...
    logging.info("[%s] Sessions adjusted from 0 to 4 minutes: %d", range_label, zero_duration_sessions_adjusted)
    ...
    metrics = RangeMetrics(
        ...
        # Total population (all users)
        mean_session_minutes_total=safe_mean(all_session_durations),
        median_session_minutes_total=safe_median(all_session_durations),
        mean_sessions_per_week_total=safe_mean([float(x) for x in all_sessions_per_week]) if all_sessions_per_week else None,
        median_sessions_per_week_total=safe_median([float(x) for x in all_sessions_per_week]) if all_sessions_per_week else None,
...
def main():
    ...
    try:
        ...
        for label, start_dt, end_dt in ranges:
            ...
            logging.info("[%s] Mean session minutes (TOTAL): %s", label, None if metrics.mean_session_minutes_total is None else round(metrics.mean_session_minutes_total, 3))
            logging.info("[%s] Median session minutes (TOTAL): %s", label, None if metrics.median_session_minutes_total is None else round(metrics.median_session_minutes_total, 3))
            logging.info("[%s] Mean sessions/week (TOTAL): %s", label, None if metrics.mean_sessions_per_week_total is None else round(metrics.mean_sessions_per_week_total, 3))
            logging.info("[%s] Median sessions/week (TOTAL): %s", label, None if metrics.median_sessions_per_week_total is None else round(metrics.median_sessions_per_week_total, 3))
            ...
            summary_rows.append({
                ...
                # Total population (all users)
                "mean_session_minutes_total": metrics.mean_session_minutes_total,
                "median_session_minutes_total": metrics.median_session_minutes_total,
                "mean_sessions_per_week_total": metrics.mean_sessions_per_week_total,
                "median_sessions_per_week_total": metrics.median_sessions_per_week_total,
